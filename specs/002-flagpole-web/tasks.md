...
- [ ] T008 `frontend/src/auth/useSession.ts`: `{session, status, signIn, signOut, handleCallback, onUnauthenticated}`; sign-out is `removeUser()` + local clear (no `signoutRedirect`, research F2); run T007 green
...
- [ ] T011 [P] `frontend/src/components/Notice.tsx` + `frontend/tests/notice.test.tsx`: `notice-loading`, `notice-error` (with `notice-retry`), `notice-success` per contracts/ui-contract.md (FR-013)
...
- [ ] T027 `frontend/src/styles.css` and basic accessibility: labels on every control, visible focus, table semantics; keep it to one stylesheet (constitution II)
- [ ] T028 [P] Run the whole suite: `npm run lint`, `npm test`, `npm run api:types:check`, `npx playwright test` (10 consecutive runs for SC-005); record the numbers in `specs/002-flagpole-web/quickstart.md`
