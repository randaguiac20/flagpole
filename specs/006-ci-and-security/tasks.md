...
- [X] T039 Run `make test`, `make test-hooks`, `make scan` and `scripts/check-ci-contract.sh` one last time and show the output; then merge to `main` — 194 unit tests + 44 frontend, hooks green, `make scan` exit 0, contract 100/100, and after the merge `scripts/verify-cluster.sh` reports 43 passed, 0 failed with `postgres` running `readOnlyRootFilesystem: true` in both namespaces (`touch /usr/local/probe` refused in the pod).
