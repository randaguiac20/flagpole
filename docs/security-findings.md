...
| 2026-09-03 | trivy | CVE-2026-76642, CVE-2026-78408, CVE-2026-78409, CVE-2026-78410 | HIGH | `python:3.12-slim@sha256:78387bc3…` — `util-linux` 2.41.5-0+deb13u1 and the libraries built from it (`bsdutils`, `libblkid1`, `liblastlog2-2`, `libmount1`, `libsmartcols1`, `libuuid1`, `login`, `mount`) | accepted | All four report `FixedVersion: none`: Debian has published nothing to move to, so `deferred` would be dishonest — the same standard as the `python:3.12-slim` row above. These are `util-linux`'s mount and login tooling, which is not in the request path: `flagpole-api` and `flagpole-consumer` run one uvicorn process as uid 10001 with a read-only root filesystem, no shell in the request path, and no local unprivileged account inside the container for a privilege-escalation issue to start from. Worth naming plainly: **nothing in this repository changed to cause this.** The same pinned digest scanned clean of these four the day before; the vulnerability database moved, and a build that was green went red on its own. That is the gate behaving correctly, and it is why triage is a file rather than a memory. **Ends when** Debian ships fixes and the image is rebuilt — Renovate's `dockerfile` manager proposes that bump, and this row goes with it. |
